...
class RecipeCard extends HTMLElement {
  ...
  set data(data) {
    ...
    let dataArticle = this.shadowRoot.querySelector('article');
    // A7. TODO - Set the contents of the <article> with the <article> template given in
    //           cardTemplate.html and the data passed in (You should only have one <article>,
    //           do not nest an <article> inside another <article>). You should use Template
    //           literals (tempalte strings) and element.innerHTML for this.
    const imageOne = document.createElement("img");
    imageOne.setAttribute("src", data.imgSrc);
    imageOne.setAttribute("alt", data.imgAlt);
    dataArticle.appendChild(imageOne);

    const receipeTitle = document.createElement("p");
    receipeTitle.setAttribute("class", "title");
    dataArticle.appendChild(receipeTitle);

    const articleLink = document.createElement("a");
    articleLink.setAttribute("href", data.titleLnk);
    articleLink.innerText = data.titleTxt;
    receipeTitle.appendChild(articleLink);

    const organization = document.createElement("p");
    organization.setAttribute("class", "organization");
    organization.innerText = data.organization;
    dataArticle.appendChild(organization);

    const rating = document.createElement("div");
    rating.setAttribute("class", "rating");
    dataArticle.appendChild(rating);

    const ratingScore = document.createElement("span");
    ratingScore.innerText = data.rating;
